...
import { RALtoChroma } from "./RAL";
import RALMatch from "./RAL";
import PANTONEMatch, { PANTONEtoChroma } from "./PANTONE";
...
export function genSmartBaseHues(
    primaryColor: string,
    algo: number = 0
): number[] {
    ...
    switch (algo) {
        case 0: {
            ...
            return selectClosestHues(output, rangeHues).sort(
                (a, b) => Math.abs(a - color[2]) - Math.abs(b - color[2])
            );
        }
        case 1: {
            let output = [
...
export function contrast(a: LCH_FORMAT, b: LCH_FORMAT) {
    return chroma.contrast(
        chroma.oklch(a[0], a[1], a[2]),
        chroma.oklch(b[0], b[1], b[2])
    );
}
...
export function toCMYK(palette: LCH_FORMAT[]) {
    return palette
        .map((p) => chroma.oklch(p[0], p[1], p[2]).cmyk())
        .map((p) => [
            p.map((v) => Math.round(v * 100) / 100),
            chroma.cmyk(p[0], p[1], p[2], p[3]).css("oklch"),
        ]);
}
...
export function fromXYZ(palette: number[][]) {
    return palette
        ...
        .map((p) => chroma.lab(p.l, p.a, p.b).oklch());
}

export function toPantone(palette: LCH_FORMAT[]) {
    return palette
        .map((p) => PANTONEMatch(p))
        .map((p) => [p.name, PANTONEtoChroma(p).css("oklch")]);
}
export function toRAL(palette: LCH_FORMAT[]) {
    return palette
        .map((p) => RALMatch(p, "design"))
        .map((p) => [`${p.code} (${p.name})`, RALtoChroma(p).css("oklch")]);
}
    ...
    return Array.from({ length: arr.length }, (_, i) => ({
        ...
        text: toCSS([
            [
                ...
                contrast(a as LCH_FORMAT, raw[i]) >
                contrast(b as LCH_FORMAT, raw[i])
                    ? a
                    : b
            ) as LCH_FORMAT,
        ]),
        compare: {
            contrast: Array.from({ length: arr.length }, (_, j) =>
                Math.abs(Math.round(contrast(raw[i], raw[j])))
            ),
            difference: Array.from({ length: arr.length }, (_, j) =>
                Math.abs(Math.round(difference(raw[i], raw[j])))
            ),
            deltaE: Array.from({ length: arr.length }, (_, j) =>
                Math.abs(Math.round(deltaE(raw[i], raw[j])))
            ),
            text: toCSS([
                [
                    [0, 0, 0],
...
export function applyBlindness(
    color: LCH_FORMAT,
    type: "protanope" | "deuteranope" | "tritanope" | "none",
    level: number = 1.0
): LCH_FORMAT {
    console.log(type);
